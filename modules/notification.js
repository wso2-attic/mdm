var notification = (function() {
	...
	module.prototype = {
		...
		getNotifications : function(ctx) {
			...
			var result = db.query(sqlscripts.notifications.select5,
					ctx.deviceid);
			...
			for (i = 0; i < result.length; i++) {
				...
				obj.received_date = common
						.getFormattedDate(result[i].received_date);
		...
		addIosNotification : function(ctx) {
			...
			var notifications = db.query(sqlscripts.notifications.select6,
					identifier);
			...
			if (notifications != null && notifications[0] != null) {
				...
				if (featureCode == "500P") {
					...
					var deviceFeatureCodes = db.query(
							sqlscripts.policy_device_profiles.select1,
							device_id);

					if (deviceFeatureCodes != null
							&& deviceFeatureCodes != undefined) {
						for ( var i = 0; i < deviceFeatureCodes.length; i++) {

							var featureName = db.query(
									sqlscripts.features.select4,
									deviceFeatureCodes[i].feature_code);

							if (featureName != null && featureName != undefined
									&& featureName[0] != null
									&& featureName[0] != undefined) {

								var payloadIdentifiers = common
										.getPayloadIdentifierMap();
								var payloadId = payloadIdentifiers[featureName[0].name];

								var data = {};
								data.identifier = payloadId;
								device.invokeMessageToIOSDevice({
									'deviceid' : device_id,
							...
							db.query(sqlscripts.policy_device_profiles.delete1,
									deviceFeatureCodes[i].id);
					...
					var pendingFeatureCodeList = db.query(
							sqlscripts.notifications.select7, notificationId);
					...
					db.query(sqlscripts.notifications.update4,
							stringify(parsedReceivedData), recivedDate,
							notificationId);
					...
					if (pendingExist) {
						...
						for ( var i = 0; i < parsedReceivedData.length; i++) {
							var receivedObject = parsedReceivedData[i];
							db.query(sqlscripts.policy_device_profiles.insert1,
									device_id, receivedObject.message.code);
						}
						db.query(sqlscripts.notifications.update5,
								notificationId);
					...
					for ( var i = 0; i < parsedReceivedData.length; i++) {
						...
						var featureName = common
								.getValueByFeatureIdentifier(payloadIdentifier);

						if (featureName == null) {
							continue;
						}

						var featureCodes = db.query(
								sqlscripts.features.select5, featureName);

						if (featureCodes == null || featureCodes[0] == null
								|| featureCodes[0].code == null) {
							continue;
						}
					...
					try {
						...
						db.query(sqlscripts.notifications.delete2, device_id,
								"501P");
					} catch (e) {
						log.info(e);
					}

					db.query(sqlscripts.notifications.update6,
							stringify(formattedData), recivedDate, identifier);
					...
					if (featureCode == "500A") {
						...
						var notifications = db.query(
								sqlscripts.notifications.select8, identifier);
						var deviceId = notifications[0].device_id;
						device.updateDeviceProperties(deviceId, osVersion,
								deviceName, wifiMac);
					}
				}
			}
